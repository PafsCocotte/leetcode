 ...
 * 制約をみるとN<=5000かつ文字の長さ10なので5000*5000*10= 2.5 * 1e8のオーダー計算量はこれでも問題なさそう。
 * 一文字違い(あるいは一文字の追加削除)の見つけ方はleetcodeでもCTCIという本でも見たはずなのに思い出せない。
 * ラビンカーブだったか、文字列のhash計算してしまうのは26^10 がわからなくても20^10 = 1e13程度なのでいけそうか
 * とも思ったが、ASCII対応128文字とか20文字とかで容易に詰むので採用しない。
...
{
public:
    bool isSub1String(const string &s1, const string &s2)
    {
        if (s1.size() != s2.size())
            return false;
        int count = 0;
    ...
    vector<string> getSub1String(const string &s, const vector<string>& wordList, vector<bool>& used)
    {
        vector<string> result;
        for (int i = 0; i < wordList.size(); i++)
        {
            if(used[i]) continue;
            if (isSub1String(s, wordList[i]))
            {
    ...
    int ladderLength(string beginWord, string endWord, vector<string> &wordList)
    {
        ...
        while(candidates.size()) {
            ...
            auto subStrings = getSub1String(current.first, wordList, used);
            for (auto& subString : subStrings) {
                candidates.emplace(subString, current.second + 1);
            }
            candidates.pop();
        }
